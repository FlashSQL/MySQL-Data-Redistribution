	...
	mtr_t*		mtr)	/*!< in/out: mini-transaction */
{
	...
	*big_rec = NULL;
	...
	if (!(flags & BTR_NO_UNDO_LOG_FLAG)) {
		/* First res erve enough free space for the file segments
	...
	if (dict_index_get_page(index)
		...
		*rec = btr_root_raise_and_insert(
		...
		//ib_logf(IB_LOG_LEVEL_INFO, "btr_page_split_and_insert : in btr_cur_pessimistic_insert");

			*rec = btr_page_split_and_insert(
					flags, cursor, offsets, heap, entry, n_ext, mtr);
	}
	if (buf_block_get_space(btr_cur_get_block(cursor)) == srv_ol_space_id &&  !dict_index_is_clust(cursor->index)){
		//ib_logf(IB_LOG_LEVEL_INFO, "btr_cur_pessimistic_insert: 1");
	}


	ut_ad(page_rec_get_next(btr_cur_get_rec(cursor)) == *rec);
		if (buf_block_get_space(btr_cur_get_block(cursor)) == srv_ol_space_id &&  !dict_index_is_clust(cursor->index)){
		//ib_logf(IB_LOG_LEVEL_INFO, "btr_cur_pessimistic_insert: 2");
	}
	...
	if (!(flags & BTR_NO_LOCKING_FLAG)) {
		...
		if (!dict_index_is_clust(index)) {
			page_update_max_trx_id(
				...
				if (buf_block_get_space(btr_cur_get_block(cursor)) == srv_ol_space_id &&  !dict_index_is_clust(cursor->index)){
		//ib_logf(IB_LOG_LEVEL_INFO, "btr_cur_pessimistic_insert: 3");
	}
		}
		if (!page_rec_is_infimum(btr_cur_get_rec(cursor))
			...
			inherit = TRUE;
				if (buf_block_get_space(btr_cur_get_block(cursor)) == srv_ol_space_id &&  !dict_index_is_clust(cursor->index)){
		//ib_logf(IB_LOG_LEVEL_INFO, "btr_cur_pessimistic_insert: 4");
	}
		}
	}
	...
	if (inherit && !(flags & BTR_NO_LOCKING_FLAG)) {
		...
		lock_update_insert(btr_cur_get_block(cursor), *rec);
			if (buf_block_get_space(btr_cur_get_block(cursor)) == srv_ol_space_id &&  !dict_index_is_clust(cursor->index)){
		//ib_logf(IB_LOG_LEVEL_INFO, "btr_cur_pessimistic_insert: 5");
	}
	}
	...
	*big_rec = big_rec_vec;
	...
	if (buf_block_get_space(btr_cur_get_block(cursor)) == srv_ol_space_id &&  !dict_index_is_clust(cursor->index)){
			ib_logf(IB_LOG_LEVEL_INFO, "btr_cur_pessimistic_insert(): function end");
	}
