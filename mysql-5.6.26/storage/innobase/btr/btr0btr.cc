				...
				be first */
{
	...
	if (page_header_get_ptr(page, PAGE_LAST_INSERT) == insert_point) {
		...
		if(buf_block_get_space(block) == srv_ol_space_id){
			//ib_logf(IB_LOG_LEVEL_INFO, "btr_page_get_split_rec_to_right: sequential inserts\n");
		/* end */}
...
split_at_new:
			...
			/* lbh */
			if(buf_block_get_space(block) == srv_ol_space_id){
				//ib_logf(IB_LOG_LEVEL_INFO, "btr_page_get_split_rec_to_right: sequential inserts: page_rec_is_supremum\n");
				*split_rec = NULL;
			/* end */}else{
				*split_rec = NULL;
			}
	...
	ulint		n_ext)	/*!< in: number of externally stored columns */
{
	...
	/* lbh */
	/* if split in order_line table and is leaf and if ins_rec key > rightmost key */
	if (buf_block_get_space(block) == srv_ol_space_id && page_is_leaf(page)) {
		do {
				//ib_logf(IB_LOG_LEVEL_INFO, "btr_page_get_split_rec:order_line split\n");
...
)
{
	buf_block_t*	block;
	page_t*		page;
	page_cur_t*	insert_page_cursor;
	buf_block_t*	insert_block;
	rec_t*		rec = NULL;
	ulint*		offsets;
	dict_index_t*	index;

	buf_block_t*	merge_block = NULL;
	...
	ulint		left_page_no;
	ulint		right_page_no;
	buf_block_t*	left_page_block = NULL;
	buf_block_t*	right_page_block = NULL;
	page_t* 	left_page;
	page_t* 	right_page;
	ulint		space;
	ulint		zip_size;
	...
	ulint		data_size;
	ulint n_moving_recs;
	rec_t* first_moving_rec;
	rec_t* tmp_rec;
	rec_t* last_moving_rec;
	ulint		n_recs;
	int incl_data = 0;
	ibool	insert_to_cur_page;
	ulint		n_uniq;
	ulint	level;
	ulint	fil_factor;
...
//0.0. cur page related info
	...
	if(dict_index_is_clust(cursor->index)){
		if(buf_block_get_space(btr_cur_get_block(cursor)) ==srv_ol_space_id){
			fil_factor = UNIV_PAGE_SIZE*15/16;
		}else{
			fil_factor = UNIV_PAGE_SIZE*31/32;
		}
...
//0.1. neighbor page related info
	...
	if((page_get_data_size(page)<=page_get_data_size(left_page)) && (page_get_data_size(page)<= page_get_data_size(right_page))){
		...
		if(page_get_data_size(left_page) > page_get_data_size(right_page)){

				//left page has more data than right page 
				...
				//check if n_moving_rec fits the page
				max_ins_size = page_get_max_insert_size(right_page, n_moving_recs);

				//since records are moving to the right sibling, (infimum next ~ nth)
				//get total size of data that are to be distributed. 
				first_moving_rec = page_rec_get_prev(page_get_supremum_rec(page));
				tmp_rec = first_moving_rec;
				incl_data = 0;
				
				ulint i=0;
				for(i=0; i<n_moving_recs; i++){
	
					offsets = rec_get_offsets(tmp_rec, cursor->index,
							offsets, ULINT_UNDEFINED,
							&heap);
					incl_data += rec_offs_size(offsets);
					...
					tmp_rec = page_rec_get_prev(tmp_rec);
				...
				if(nth_rec < n_recs - n_moving_recs){
					...
					if(incl_data + page_get_data_size(right_page)+rec_get_converted_size(cursor->index, tuple, n_ext) >=fil_factor){
						...
						while(incl_data + page_get_data_size(right_page)+rec_get_converted_size(cursor->index, tuple, n_ext) >=fil_factor){
							...
							incl_data-= rec_offs_size(offsets);
							last_moving_rec = page_rec_get_next(last_moving_rec);
							n_moving_recs--;
				...
				//check if n_moving_recs fit the page
				max_ins_size = page_get_max_insert_size(left_page, n_moving_recs);

				//since records are moving to the left sibling, (nth sumpremum_prev)
				//get total size of data that are to be distributed. 
				first_moving_rec = page_rec_get_next(page_get_infimum_rec(page));
				tmp_rec = first_moving_rec;
				incl_data = 0;			
				...
				if(nth_rec < n_moving_recs){
					...
					if(incl_data + page_get_data_size(left_page)+rec_get_converted_size(cursor->index, tuple, n_ext) >=fil_factor){
						...
						while(incl_data + page_get_data_size(left_page)+rec_get_converted_size(cursor->index, tuple, n_ext) >=fil_factor){
							...
							incl_data-= rec_offs_size(offsets);
							last_moving_rec = page_rec_get_prev(last_moving_rec);
							n_moving_recs--;
							if(n_moving_recs<3){
								goto err_exit;
							}
					...
					ib_logf(IB_LOG_LEVEL_INFO, "redistribute to left, insert to cur page");
					insert_to_cur_page = TRUE;
					if(incl_data < rec_get_converted_size(cursor->index, tuple, n_ext) || page_get_data_size(page)+rec_get_converted_size(cursor->index, tuple, n_ext)-incl_data >=fil_factor){
						//ib_logf(IB_LOG_LEVEL_INFO, "redistribute to left, cur page too full");
						goto err_exit;
					}
				}
				if(nth_rec == n_moving_recs){
		...
		if(btr_page_get_level(buf_block_get_frame(merge_block), mtr)
	      != btr_page_get_level(buf_block_get_frame(block), mtr)){
			  goto err_exit;

		  }
		ut_ad(mtr_memo_contains(mtr, merge_block, MTR_MEMO_PAGE_X_FIX));
	}

	if(insert_to_cur_page){
		insert_block = block;
	}else{
		insert_block = merge_block;
	}
	...
	if (merge_page_zip) {
		...
		if (is_left) { 
			...
			ulint	orig_pred = page_move_rec_list_start(
				merge_block, block, last_moving_rec,
				index, mtr);

			if (!orig_pred) {
				//ib_logf(IB_LOG_LEVEL_INFO, "redistribute : move record list to left page failed");
			...
			//ib_logf(IB_LOG_LEVEL_INFO, "last moving rec: %lu, first rec: %lu, last rec: %lu, last rec of left page: %lu, first rec of left page: %lu", last_moving_rec, page_rec_get_next(page_get_infimum_rec(page)), page_rec_get_prev(page_get_supremum_rec(page)), page_rec_get_prev(page_get_supremum_rec(left_page)), page_rec_get_next(page_get_infimum_rec(left_page)));
			...
			//ib_logf(IB_LOG_LEVEL_INFO, "redistribute : move record list to left page");
			//ib_logf(IB_LOG_LEVEL_INFO, "last moving rec: %lu, first rec: %lu, last rec: %lu, last rec of left page: %lu, first rec of left page: %lu", last_moving_rec, page_rec_get_next(page_get_infimum_rec(page)), page_rec_get_prev(page_get_supremum_rec(page)), page_rec_get_prev(page_get_supremum_rec(left_page)), page_rec_get_next(page_get_infimum_rec(left_page)));

		}else{
			//move record list to right page

			ibool		orig_succ;

			offsets1 = btr_page_get_father_block(NULL, heap, 
				cursor->index, right_page_block, mtr, &next_father_cursor);

			//ib_logf(IB_LOG_LEVEL_INFO, "first rec: %lu, last rec: %lu, last rec of right page: %lu, first rec of right page: %lu", page_rec_get_next(page_get_infimum_rec(page)), page_rec_get_prev(page_get_supremum_rec(page)), page_rec_get_prev(page_get_supremum_rec(right_page)), page_rec_get_next(page_get_infimum_rec(right_page)));
				

			orig_succ = page_move_rec_list_end(merge_block, block,
							last_moving_rec,
							cursor->index, mtr);
			...
			//ib_logf(IB_LOG_LEVEL_INFO, "last moving rec: %lu, first rec: %lu, last rec: %lu", last_moving_rec, page_rec_get_next(page_get_infimum_rec(right_page)),page_rec_get_prev(page_get_supremum_rec(right_page)));
			//ib_logf(IB_LOG_LEVEL_INFO, "first rec: %lu, last rec: %lu, last rec of right page: %lu, first rec of right page: %lu", page_rec_get_next(page_get_infimum_rec(page)), page_rec_get_prev(page_get_supremum_rec(page)), page_rec_get_prev(page_get_supremum_rec(right_page)), page_rec_get_next(page_get_infimum_rec(right_page)));
			...
			//ib_logf(IB_LOG_LEVEL_INFO, "redistribute : move record list to right page end");
			//ib_logf(IB_LOG_LEVEL_INFO, "first rec: %lu, last rec: %lu, last rec of right page: %lu, first rec of right page: %lu", page_rec_get_next(page_get_infimum_rec(page)), page_rec_get_prev(page_get_supremum_rec(page)), page_rec_get_prev(page_get_supremum_rec(right_page)), page_rec_get_next(page_get_infimum_rec(right_page)));
		}	
	...
	rec = page_cur_tuple_insert(insert_page_cursor, tuple, cursor->index,
					rec_offsets, &heap, n_ext, mtr);

	if (rec == NULL) {
		ib_logf(IB_LOG_LEVEL_INFO, "cur page no:%lu, merge page no: %lu, insert cur rec failed", buf_block_get_page_no(block), buf_block_get_page_no(merge_block)); 
		mem_heap_empty(heap);
		rec = btr_page_split_and_insert(flags, cursor, rec_offsets, &heap, tuple, n_ext, mtr);
	}
	...
	if(is_left){
		//adjust cursor to cur page infimum
		compressed = btr_cur_pessimistic_delete(
			&err, TRUE, &father_cursor,
			BTR_CREATE_FLAG, RB_NONE, mtr);
		...
		dtuple_t*	node_ptr = dict_index_build_node_ptr(
			cursor->index, page_rec_get_next(page_get_infimum_rec(page)), buf_block_get_page_no(block),
			heap, level);

		btr_insert_on_non_leaf_level(
			flags, cursor->index, level + 1, node_ptr, mtr);

		//delete cur page father cursor (cur page infimum changed)
		//build new ptr for cur page infimum rec and insert to father page
		...
		dtuple_t*	node_ptr = dict_index_build_node_ptr(
			cursor->index, page_rec_get_next(page_get_infimum_rec(right_page)), buf_block_get_page_no(right_page_block),
			heap, level);
...
//7. update insert buffer bitmap
		...
		if (!dict_index_is_clust(cursor->index) && page_is_leaf(merge_page)) {
			...
			//ib_logf(IB_LOG_LEVEL_INFO, "redistribute : ibuf_update_free_bits_zip");
			if (zip_size) {
		...
		//ib_logf(IB_LOG_LEVEL_INFO, "function end : dict_index_is_clust(cursor->index): %lu", dict_index_is_clust(cursor->index));
		//ib_logf(IB_LOG_LEVEL_INFO, "end : insert_to_cur_page: %lu",insert_to_cur_page);
		ib_logf(IB_LOG_LEVEL_INFO, "end : cur_page_no: %lu, cur_page data size: %lu, cur_page n_recs: %lu", buf_block_get_page_no(block), page_get_data_size(page), page_get_n_recs(page));
		ib_logf(IB_LOG_LEVEL_INFO, "end : left_page_no: %lu, left_page data size: %lu, left_page n_recs: %lu", left_page_no, page_get_data_size(left_page), page_get_n_recs(left_page));
		ib_logf(IB_LOG_LEVEL_INFO, "end : right_page_no: %lu, right_page data size: %lu, right_page n_recs: %lu", right_page_no, page_get_data_size(right_page), page_get_n_recs(right_page));
		...
		if (zip_size&& page_is_leaf(page) && !dict_index_is_clust(cursor->index)) {
			...
			ib_logf(IB_LOG_LEVEL_INFO, "insert cur rec failed 2");
			goto err_exit;
		}
