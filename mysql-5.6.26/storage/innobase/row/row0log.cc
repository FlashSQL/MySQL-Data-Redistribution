						...
						duplicate key errors */
{
	...
	if ( !dict_index_is_clust(index)){
		ib_logf(IB_LOG_LEVEL_INFO, "row_ins_sec_index_entry_low() in row_log_table_apply_insert_low()");
	}
...
func_exit_committed:
	...
	while ((index = dict_table_get_next_index(index)) != NULL) {
		...
		entry = row_build_index_entry(row, NULL, index, heap);
			if ( !dict_index_is_clust(index)){
		ib_logf(IB_LOG_LEVEL_INFO, "row_ins_sec_index_entry_low() in row_log_table_apply_update()");
	}
		error = row_ins_sec_index_entry_low(
			BTR_CREATE_FLAG | BTR_NO_LOCKING_FLAG
			| BTR_NO_UNDO_LOG_FLAG | BTR_KEEP_SYS_FLAG,
...
insert_the_rec:
			...
			*error = btr_cur_optimistic_insert(
			...
			if (buf_block_get_space(btr_cur_get_block(&cursor)) == srv_ol_space_id &&  !dict_index_is_clust(index)){
					ib_logf(IB_LOG_LEVEL_INFO, "btr_cur_pessimistic_insert in rov0log.cc");
			}
